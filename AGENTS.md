...
13. **Use AI prompt automation scripts** — When batch processing source files (C/C++/Obj-C), use `batch_ai_prompt.sh` (Linux/macOS) or `batch_ai_prompt.bat` (Windows) to iterate over all `.c`, `.h`, `.cpp`, `.hpp`, `.cc`, `.cxx`, `.m`, `.mm` files and execute review prompts one-by-one. These scripts handle spaces in filenames and ensure sequential processing.
