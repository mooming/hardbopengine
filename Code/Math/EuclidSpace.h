...
namespace HE
{
  ...
  public:
    ...
    {
      auto pCurrentParent = This::pParent;
      auto currentWorld = World();

      if (pCurrentParent != nullptr)
      {
        pCurrentParent->pChildren.Remove(this);
      }
      ...
      {
        pParent->pChildren.Add(this);
        local = pParent->World().InverseTransform(currentWorld);
      }

      Invalidate();
    }

    void SetParentKeepLocal(EuclidSpace* pParent)
    {
      This::pParent = pParent;
