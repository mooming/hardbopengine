...
namespace HE
{
    ...
    public:
        ...
        inline bool operator!= (const This& rhs) { return !(*this == rhs); }
        template <typename T>
        inline T operator* (const T& rhs) const { return Transform(rhs); }
    ...
    template<typename T, int N>
    inline RigidTransform<T, N>::operator Mat3x3() const
    {
        return GetR();
    }

    template<typename T, int N>
    inline RigidTransform<T, N>::operator Mat4x4() const
    {
        return GetTR();
    }
}
