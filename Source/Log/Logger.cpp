...
void FallbackLog(
    StaticString category, ELogLevel level, const Logger::TLogFunction& logFunc)
{
    ...
    StaticString category, ELogLevel level, const TLogFunction& logFunc)
{
    ...
    static TAtomicConfigParam<uint8_t> CPLogLevel("Log.Level",
        "The Default Log Level", static_cast<uint8_t>(ELogLevel::Info));
...

    {
        ...
        inputBuffer.emplace_back(
            level, threadName, category, ls.c_str(), ls.Size());
