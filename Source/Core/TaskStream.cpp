...
namespace hbe
{
	...
	TaskStream::TaskStream() :
		deltaTime(0.0f), flipCount(0), isDirty(false), isResidentListDirty(false), hasCancelledTask(false)
	...
	TaskStream::TaskStream(StaticString name) :
		name(name), deltaTime(0.0f), flipCount(0), isDirty(false), isResidentListDirty(false), hasCancelledTask(false)
	{
		Assert(threadID == std::thread::id());
	...
	{
		{
			...
			auto log = Logger::Get(name);
			log.Out([&](auto& ls)
			{ ls << "Request: Key = " << key << ", Task = " << task.GetName() << '[' << start << ", " << end << ')'; });
	...
	{
		{
			...
			auto log = Logger::Get(name);
			log.Out([&](auto& ls) { ls << "AddResident: Key = " << key << ", Task = " << task.GetName(); });
	...
	{
		{
			...
			if (likely(task != nullptr))
			{
				log.Out([&](auto& ls)
				{
					ls << "RemoveResidentTask: Key = " << key << ", Task = " << task->GetName() << '[' << found->start
					   << ", " << found->end << ')';
				});
	...
	void TaskStream::RemoveResidentTaskSync(TKey key)
	{
...

		{
			...
			else
			{
				log.Out([&](auto& ls)
				{
					ls << "RemoveResidentSyncSync: Key = " << key << ", Task = Null" << '[' << found->start << ", "
					   << found->end << ')';
				});
			}
	...
	void TaskStream::RunLoop(const std::atomic<bool>& isRunning)
	{
		...
		auto IsDirty = [this]() { return isDirty.load(std::memory_order_relaxed) || residents.size() > 0; };
