...
AllocStats::AllocStats()
    : name(""), isInline(false), capacity(0), usage(0), maxUsage(0), totalRequested(0),
      maxRequested(0), totalFallback(0), maxFallback(0), allocCount(0), deallocCount(0),
      fallbackCount(0)
{
}
...
void AllocStats::Print()
{
    ...
    log.Out(
        ...
        {
            ls << "fallback = "
               << (fallbackCount > 0 ? totalFallback / fallbackCount : totalFallback) << " / "
    ...
    log.Out(
        [this](auto& ls)
        {
            ls << "alloc/dealloc/fallback = " << allocCount << " / " << deallocCount << " / "
               << fallbackCount << '(' << (allocCount > 0 ? fallbackCount * 100 / allocCount : 0)
               << ')';
        });
}
