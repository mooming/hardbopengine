...
MultiPoolAllocator::MultiPoolAllocator(
    const char* inName, TInitializerList initialConfigurations, size_t allocationUnit,
    size_t minBlockSize)
...
MultiPoolAllocator::~MultiPoolAllocator()
{
    ...
    {
        mmgr.Update(
            bank.GetID(), [&currentTID](auto& proxy) { proxy.threadId = currentTID; },
...
void MultiPoolAllocator::PrintUsage() const
{
    ...
    for (auto& pool : banks)
    {
        log.Out(
            [&pool](auto& ls)
            {
                ls << '[' << pool.GetBlockSize() << "] Memory = " << pool.GetUsage() << " / "
                   << pool.GetCapacity()
...
void MultiPoolAllocator::GenerateBank(size_t blockSize, size_t numberOfBlocks)
{
    ...
    log.Out(
        ELogLevel::Verbose,
        [&str](auto& ls) { ls << "The bank[" << str.c_str() << "] has been generated. "; });

    const auto incCapacity = blockSize * numberOfBlocks;
    auto& mmgr = MemoryManager::GetInstance();
    mmgr.Update(
        GetID(), [incCapacity](auto& allocProxy) { allocProxy.stats.capacity += incCapacity; },
        "A new bank is added.");
}
...
void MultiPoolAllocatorTest::Prepare()
{
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            AllocatorScope scope(allocator);
            void* pointers[] = {
                mmgr.AllocateBytes(0),    mmgr.AllocateBytes(8),   mmgr.AllocateBytes(16),
            ...
            {
                ls << "Fallback count mismatched. FallbackCount = " << stat.fallbackCount
                   << ", but 2 expected." << lferr;
            }
#endif // PROFILE_ENABLED
        });
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            ls << "Performance: MultiPoolAllocator = " << heSec << " sec, std malloc = " << stdSec
               << " sec" << lf;

            if (heSec > stdSec)
            {
                ls << "MultiPoolAllocator is slower than std malloc."
                   << " MultiPoolAllocator  = " << heSec << " sec, std malloc = " << stdSec
                   << " sec" << lfwarn;
            }
        });
}
