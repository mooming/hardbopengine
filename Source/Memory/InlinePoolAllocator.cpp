...
{
    AddTest(
        "Zero-sized Allocation & Deallocation",
        [](auto&)
        {
    ...
    auto allocDeallocTest = [](auto& inlineAlloc, auto& ls, auto& lf) -> float
    {
        ...
        ls << "Performance: " << inlineTimeSec << " msec vs STL: " << stdTimeSec
           << " msec, rate = [" << rate << "], fallback count = " << stat.fallbackCount << " / "
