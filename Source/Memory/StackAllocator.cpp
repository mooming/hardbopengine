...
StackAllocator::StackAllocator(const char* name, SizeType inCapacity, const TSrcLoc location)
    : id(InvalidAllocatorID), parentID(InvalidAllocatorID), capacity(inCapacity), cursor(0),
      buffer(nullptr), srcLocation(location)
...
void* StackAllocator::Allocate(const size_t requested)
{
    ...
    if (unlikely(size > freeSize))
    {
        auto& mmgr = MemoryManager::GetInstance();
        mmgr.LogError(
            [this, size, freeSize](auto& lout)
            {
                lout << "StackAllocator: Not Enough Memory! "
                     << "require = " << size << " : free = " << freeSize << " / " << capacity;
            });
...
void StackAllocator::Deallocate(const Pointer ptr, const SizeType requested)
{
    ...
    if (unlikely(expected != provided))
    {
        auto& mmgr = MemoryManager::GetInstance();
        mmgr.LogError(
            [this, ptr, expected, provided](auto& lout)
            {
...
void StackAllocatorTest::Prepare()
{
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            if (stack.GetUsage() != 0)
            {
                ls << "Deallocation Failed. Usage should be zero, but " << stack.GetUsage()
                   << lferr;
            }
        });
...

            {
...

                {
...

                    {
                        ...
                        const auto depth = depthSeed++;
                        ls << "Neted Level " << depth
                           << ", free size = " << scope3.GetAllocator().GetAvailable() << " / "
...

                        {
                            ...
                            const auto depth = depthSeed++;
                            ls << "Neted Level " << depth
                               << ", free size = " << scope4.GetAllocator().GetAvailable() << " / "
...

                            {
                                ...
                                const auto depth = depthSeed++;
                                ls << "Neted Level " << depth
                                   << ", free size = " << scope5.GetAllocator().GetAvailable()
