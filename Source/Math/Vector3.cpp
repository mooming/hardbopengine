...
void HE::Vector3Test::Prepare()
{
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            if (xCrossY != Float3::Z)
            {
                ls << "Incorrect Cross Product Result: X x Y = " << xCrossY << ", but " << Float3::Z
                   << " expected." << lferr;
            }
            ...
            {
                ls << "Incorrect Cross Product Result: Forward x Right = " << fCrossR << ", but "
                   << Float3::Up << " expected." << lferr;
            }
            ...
            {
                ls << "Incorrect Cross Product Result: Right x Forward = " << rCrossF << ", but "
                   << Float3::Up << " expected." << lferr;
            }
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
...

            {
                ...
                ls << "Float3 Dot Time = " << Time::ToFloat(heTime) << ", Result = " << dotResult
                   << lf;
            }
        });
    ...
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            {
                ls << "Float3 slerp 1 should match its origin " << x << ", but "
                   << Float3::Slerp(x, y, 0.0f) << lferr;
            }

            if (y != Float3::Slerp(x, y, 1.0f))
            {
                ls << "Float3 slerp 1 should match its destination " << y << ", but "
                   << Float3::Slerp(x, y, 1.0f) << lferr;
            }
