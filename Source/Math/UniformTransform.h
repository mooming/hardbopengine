...
namespace HE
{
    ...
    public:
        ...
        UniformTransform(const Mat4x4 &mat)
        {
            ...
            scale =
                (c1.Normalize() + c2.Normalize() + c3.Normalize()) * oneThird;
        ...
        This Transform(const This &rhs) const
        {
            ...
            result.translation =
                rotation * (scale * rhs.translation) + translation;
    ...
    LogStream &operator<<(LogStream &os, const UniformTransform<T> &local)
    {
        ...
        os << "Position: (" << local.translation.x << ", "
           << local.translation.y << ", " << local.translation.z << ")"
           << hendl;

        auto r = local.rotation.EulerAngles();
        os << "Rotation: (" << r.x << ", " << r.y << ", " << r.z << ")"
           << hendl;
