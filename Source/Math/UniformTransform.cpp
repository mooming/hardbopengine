...
void hbe::UniformTransformTest::Prepare()
{
    ...
    AddTest("Inverse Matrix", [this](auto& ls) {
        ...
        if (!tm3.IsInvertible())
        {
            ls << "Transform matrix should be invertible." << std::endl
               << tm3 << lferr;
        }
        ...
        {
            ls << "Transform matrix inverse failed. " << result << ", but "
        ...
        if (result != Float3::Forward)
        {
            ls << "Inverse transform failed. " << result << ", but "
               << Float3::Forward << " expected." << lferr;
        }

        if (tm3.Inverse().Inverse() != tm3)
        {
            ls << "Inverse of inverse transform failed. "
               << tm3.Inverse().Inverse() << ", but " << tm3 << " expected."
               << lferr;
        }
    });
}
