...
void RigidTransformTest::Prepare()
{
	AddTest("Default RigidTransform & Constants", [this](auto& ls)
	{
		...
		{
			ls << "Identity Transform Failed. " << (tm * Float3::Forward) << ", but " << Float3::Forward << " expected."
			   << lferr;
		}
	});
	...
	AddTest("RigidTransform Operations", [this](auto& ls)
	{
		...
		if (result1 != result2)
		{
			ls << "Transform result failed. " << (tm3 * Float3::Forward) << ", but " << result2 << " expected."
			   << lferr;
		}
		...
		{
			ls << "Transform matrix inverse failed. " << result << ", but " << Float3::Forward << " expected." << lferr;
		}

		result = tm2.InverseTransform(tm.InverseTransform(result2));
		if (result != Float3::Forward)
		{
			ls << "Inverse transform failed. " << result << ", but " << Float3::Forward << " expected." << lferr;
		}
