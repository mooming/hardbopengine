...
void HE::Matrix3x3Test::Prepare()
{
    AddTest(
        ...
        [this](auto& ls)
        {
            ...
            {
                ls << "Rotation around right failed. " << (rRight * right) << ", but " << right
                   << " expected." << lferr;
            }

            if (rUp * up != up)
            {
                ls << "Rotation around Y failed. " << (rUp * up) << ", but " << up << " expected."
                   << lferr;
            }

            if (rForward * forward != forward)
            {
                ls << "Rotation around Z failed. " << (rForward * forward) << ", but " << forward
                   << " expected." << lferr;
            }

            if (rRight * up != -forward)
            {
                ls << "Rotation around X failed. " << (rRight * up) << ", but " << forward
                   << " expected." << lferr;
            }

            if (rUp * forward != -right)
            {
                ls << "Rotation around Y failed. " << (rUp * forward) << ", but " << right
                   << " expected." << lferr;
            }

            if (rForward * right != -up)
            {
                ls << "Rotation around Z failed. " << (rForward * right) << ", but " << up
                   << " expected." << lferr;
            }
        });
}
