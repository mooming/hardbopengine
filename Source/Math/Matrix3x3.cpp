...
void HE::Matrix3x3Test::Prepare()
{
    AddTest("Matrix3x3 Test", [this](auto& ls)
    {
        ...
        if (rUp * up != up)
        {
            ls << "Rotation around Y failed. "
                << (rUp * up) << ", but " << up << " expected." << lferr;
        }
        ...
        {
            ls << "Rotation around X failed. " << (rRight * up)
                << ", but " << forward << " expected." << lferr;
        }

        if (rUp * forward != - right)
        {
            ls << "Rotation around Y failed. " << (rUp * forward)
                << ", but " << right << " expected." << lferr;
        }

        if (rForward * right != - up)
        {
            ls << "Rotation around Z failed. " << (rForward * right)
                << ", but " << up << " expected." << lferr;
        }
    });
}
