...
class Quaternion
{
  ...
  public:
    ...
    inline Quaternion(const Mat3x3& mat)
    {
        ...
        Assert(
            IsEqual(mat.Determinant(), 1.0f), "[Quaternion] Matrix Determinant should be 1, but ",
            mat.Determinant());
    ...
    inline Vec3 EulerAngles() const
    {
        ...
        angle.y =
            std::abs(sinp) >= 1 ? std::copysign(HalfPi, sinp) : RadianToDegree(RotationAsin(sinp));
    ...
    {
        AssertMessage(
            from.IsUnity(), "Quaternion slerp should have unit length", ", but ",
            from.vector.Length());
        AssertMessage(
            to.IsUnity(), "Quaternion slerp should have unit length", ", but ", to.vector.Length());
