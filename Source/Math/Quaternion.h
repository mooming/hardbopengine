...
namespace HE
{
    ...
    public:
        ...
        inline Quaternion(const Mat3x3& mat)
        {
            ...
            {
                const float tmp =
                    sqrt(1.0f - mat.m11 + mat.m22 - mat.m33) * 0.5f;
                const float reciprocal = 0.25f / tmp;
        ...
        inline Vec3 EulerAngles() const
        {
            ...
            const float yy = y * y;
            angle.x = RadianToDegree(RotationAtan2(
                2.0f * (w * x + y * z), 1.0f - 2.0f * (x * x + yy)));

            const auto sinp = 2.0f * (w * y - x * z);
            angle.y = std::abs(sinp) >= 1 ? std::copysign(HalfPi, sinp)
                                          : RadianToDegree(RotationAsin(sinp));
            angle.z = RadianToDegree(RotationAtan2(
                2.0f * (w * z + x * y), 1.0f - 2.0f * (yy + z * z)));
