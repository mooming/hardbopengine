...
class Matrix3x3
{
  ...
  public:
    ...
    {
        return m11 * m22 * m33 + m12 * m23 * m31 + m13 * m21 * m32 - m11 * m23 * m32 -
    ...
    inline bool IsOrthogonal() const
    {
        return IsZero(rows[0].Dot(rows[1])) && IsZero(rows[1].Dot(rows[2])) &&
               IsZero(rows[2].Dot(rows[0])) && rows[0].IsUnity() && rows[1].IsUnity() &&
               rows[2].IsUnity();
    }
    ...
    inline void SetEulerAngles(const Vec& euler) { SetEulerAngles(euler.x, euler.y, euler.z); }
