...
void HE::AABBTest::Prepare()
{
    ...
    AddTest(
        ...
        [&, this](auto& ls)
        {
            ...
            {
                ls << "Invalid Intersection of AABB. " << hendl << "Intersection of " << hendl
                   << bbox2 << hendl << " and " << hendl << bbox3 << hendl << "is "
                   << bbox2.Intersection(bbox3) << hendl << "But expecting the empty." << lferr;
            }
        });
}
