...
{
    FatalAssert(
        numHardwareThreads > 0, "It should have at least one hardware thread.");
}
...
void TaskSystem::Initialize()
{
    ...
    auto logFilter = [](auto level) {
        static TAtomicConfigParam<uint8_t>   CPLogLevel("Log.TaskSystem",
...
TaskHandle TaskSystem::DispatchTask(
    StaticString taskName, Runnable func, TIndex workerStreamIndex)
{
    TaskHandle handle;
    ...
    if (unlikely(!streams.IsValidIndex(workerStreamIndex)))
    {
        log.OutWarning([workerStreamIndex, taskName](auto& ls) {
            ls << "Invalid stream index " << workerStreamIndex << " for "
               << taskName.c_str();
        });
    ...
    for (TIndex index = 0; index < length; ++index)
    {
        ...
        auto& stream = streams[workerStreamIndex];
...
void TaskSystemTest::Prepare()
{
    ...
    auto countPrimeNumbers = [](auto& ls, auto& lf, auto& lferr,
        ...
        Time::TDuration duration;
        {
            ...
            handle =
                taskSys.DispatchTask("PNCounter", upperBound, func, numWorkers);
        ...
        ls << "# of Prime Numbers = " << count << ", done = " << task.NumDone()
           << "/" << task.NumStreams() << ", time = " << Time::ToFloat(duration)
           << " seconds" << lf;
