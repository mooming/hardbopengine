...
#ifdef PROFILE_ENABLED
    ScopedLock(
        TLockable& lockable, float timeOutSec = 0.01f, const TSrcLoc& srcLoc = TSrcLoc::current())
        : lockable(lockable), startTime(Time::TStopWatch::now()), timeOutSec(timeOutSec),
          srcLoc(srcLoc)
    {
        lockable.lock();
#ifdef __DEBUG__
            ...
            log.OutWarning(
                [&, this](auto& ls)
                {
                    ls << "[file: " << srcLoc.file_name() << '(' << srcLoc.line() << ':'
                       << srcLoc.column() << ") " << srcLoc.function_name() << ": TimedOut! "
                       << duration << " sec exceeds its timelimt " << timeOutSec;
                });
        }
#endif // PROFILE_ENABLED
    }
