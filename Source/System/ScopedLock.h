...
template <typename T>
concept CLockable = requires(T t) { t.lock(); t.unlock(); };
...
#ifdef PROFILE_ENABLED
    ScopedLock(TLockable& lockable, float timeOutSec = 0.01f, const TSrcLoc& srcLoc = TSrcLoc::current())
        : lockable(lockable)
        ...
        if (unlikely(timeOutSec > 0.0f && duration > timeOutSec))
        {
            ...
            log.OutWarning([&, this](auto& ls)
            {
                ls << "[file: " << srcLoc.file_name() << '(' << srcLoc.line()
                    ...
                    << ": TimedOut! " << duration
                    << " sec exceeds its timelimt " << timeOutSec;
            });
        }
#endif // PROFILE_ENABLED
    }
