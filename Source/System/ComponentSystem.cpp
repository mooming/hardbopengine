    ...
    AddTest("Update Component Test", [this](auto& ls)
    {
        ...
        struct Test : public Component
        {
            ...
            virtual void Init() override
            {
                if (GetState() != ComponentState::BORN)
                {
                    ls << "State failure, state = " << GetState()
                        << ", but expected " << ComponentState::BORN << lferr;
            ...
            virtual void Update(const float deltaTime) override
            {
                ...
                {
                    ls << "State failure, state = " << GetState()
                        << ", but expected " << ComponentState::ALIVE << lferr;
                    testResult = false;
                }
            ...
            virtual void Release() override
            {
                ...
                {
                    ls << "State failure, state = " << GetState()
                        << ", but expected " << ComponentState::DEAD << lferr;
            ...
            virtual void OnDisable() override
            {
                ...
                {
                    ls << "State failure, state = " << GetState()
                        << ", but expected " << ComponentState::SLEEP << lferr;
