...
#define SNAKE_BLOCK_SIZE_IN_PIXELS 1 
...
static int handle_key_event_(SnakeContext *ctx, SDL_Scancode key_code)
{
    ...
    case SDL_SCANCODE_RIGHT:
        snake_redir(ctx, SnakeDirection::SNAKE_DIR_RIGHT);
        break;
    case SDL_SCANCODE_UP:
        snake_redir(ctx, SnakeDirection::SNAKE_DIR_UP);
        break;
    case SDL_SCANCODE_LEFT:
        snake_redir(ctx, SnakeDirection::SNAKE_DIR_LEFT);
        break;
    case SDL_SCANCODE_DOWN:
        snake_redir(ctx, SnakeDirection::SNAKE_DIR_DOWN);
        break;
    default:
        break;
    }
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
	...
	for (i = 0; i < SNAKE_GAME_WIDTH; i++)
	{
        for (j = 0; j < SNAKE_GAME_HEIGHT; j++)
		{
            ...
            if (ct == SnakeCell::SNAKE_CELL_NOTHING)
                continue;
