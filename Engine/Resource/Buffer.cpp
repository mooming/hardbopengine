...
namespace hbe
{
	...
	Buffer::Buffer(Buffer&& rhs) noexcept : size(rhs.size), data(rhs.data), releaser(std::move(rhs.releaser))
	{
		rhs.size = 0;
		rhs.data = nullptr;
	...
	Buffer::Buffer(const TGenerateBuffer& genFunc) : size(0), data(nullptr) { genFunc(size, data); }

	Buffer::Buffer(const TGenerateBuffer& genFunc, const TReleaseBuffer& releaseFunc) : size(0), data(nullptr)
	{
		genFunc(size, data);
	...
	void BufferTest::Prepare()
	{
		...
		AddTest("Memory Buffer", [this](auto& ls)
		{
			...
			for (int i = 0; i < TestSize; ++i)
			{
				...
				{
					ls << "An invalid initial value: " << ptr[i] << ", but " << InitialValue << " is expected."
					   << lferr;
				}
			}
		});
		...
		AddTest("File Buffer", [this](auto& ls)
		{
...

			{
				...
				if (wSize != TestSize)
				{
					ls << "Failed to write a file with the path = " << path << ", " << TestSize << " bytes. " << wSize
...

			{
				...
				for (int i = 0; i < TestSize; ++i)
				{
					ls << "ptr[" << i << "] = " << ptr[i] << " <=> text[" << i << "] = " << text[i] << lf;

					if (ptr[i] != text[i])
					{
						ls << "Invalid character (" << ptr[i] << "), but (" << text[i] << ") is expected." << lferr;
					}
...

			{
				...
				for (int i = 0; i < TestSize; ++i)
				{
					...
					if (ch != 'a')
					{
						ls << "Invalid character is read. ch = (" << ch << "), but 'a' is expected." << lferr;
					}
				}
