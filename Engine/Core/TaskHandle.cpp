...
namespace hbe
{
	...
	TaskHandle::TaskHandle(TaskHandle&& rhs) : key(rhs.key), taskIndex(rhs.taskIndex), releaser(std::move(rhs.releaser))
	{
		rhs.key = InvalidKey;
		rhs.taskIndex = -1;
	...
	bool TaskHandle::IsValid() const { return key != InvalidKey && taskIndex >= 0; }
