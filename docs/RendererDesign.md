...
// Note: __attribute__((abi("hlsl3_0"))) is used for custom compiler ABI alignment to ensure 
// compatibility between CPU-side structures and cross-compiled shader bytecode.
...
*(Note: Z-sorting precision is coarse-grained; depth is shifted right by 16 bits to prioritize PSO and Material grouping in the radix sort).*
...
| Capability | DirectX 12 (Tier 1/2) | Vulkan (Descriptor Indexing) | Metal (Tier 1/2) |
| :--- | :--- | :--- | :--- |
| **Bindless Textures** | Large (Heap size dependent) | Large (Descriptor Set limit) | Limited (Argument Buffer size) |
| **Descriptor Update** | CPU-side `SetDescriptorTable` | `vkUpdateDescriptorSets` | CPU-side `MTLArgumentBuffer` updates |
...
### 4. Critical Edge Cases & Resource Management
...
- **Aliasing:** The Render Graph compiler performs overlap analysis to allow multiple transient textures/buffers to share the same physical memory if their lifetimes do not intersect.
...
## 5. System Synchronization & Pipeline Timeline

The rendering pipeline follows a strict temporal sequence to ensure data consistency and maximize parallelism.

### Pipeline Timeline (per Frame)

1.  **[CPU] Game Thread (Logic/ECS):** Updates actor transforms, animation, and physics. Produces immutable ECS snapshot.
2.  **[CPU] RHI Thread (EXTRACT):** Locklessly copies data from ECS snapshot to RHI SoA buffers.
3.  **[CPU/GPU] RHI Worker Threads (PREPARE):** Parallel compute shaders execute frustum culling, occlusion testing, and TLAS builds.
4.  **[CPU] RHI Thread (RENDER GRAPH BUILD):** Builds the DAG, resolves dependencies, and injects barriers.
5.  **[CPU] RHI Thread (SUBMIT):** Radix sorts command buckets and submits to native APIs (DX12/VK/Metal) via thread-local lists.
6.  **[GPU] Hardware Execution:** Executes graphics and compute queues as per command buffer instruction.

### Synchronization Primitives

- **Fences:** Used for CPU-GPU synchronization to signal frame completion and reclaim transient resources.
- **Barriers:** Injected by the Render Graph to manage resource transitions (e.g., `UAV` $\to$ `ShaderResource`) and memory visibility.
- **Semaphores/Events:** Used for inter-queue synchronization (e.g., Compute $\to$ Graphics dependency).
